...
class PieceMover
  ...
  def best_target
    @best_target ||= site.neighbors.map do |neighbor|
      score = neighbor.discounted_score +
              (10_000 * [0, (neighbor.planned_strength + @site.strength - 255)].max) +
              (@decisionmaker.borders.length > 0 && neighbor.being_a_wall?(@interacted_enemies) ? 10_000_000 : 0 )
      # break score ties by choosing the 'better' path:
      # lower production in friendly sites, higher production in neutral
      [score, (neighbor.friendly? ? 1 : -1) * neighbor.production + rand, neighbor]
    end.sort.first
  end
