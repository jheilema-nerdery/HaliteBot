...
class PieceMover
  ...
  def initialize(site, map, multiplier, search_distance)
    @site = site
    @map = map
  ...
  def calculate_move
    if site.strength == 0 || (site.is_weak?(@multiplier) && stillness_allowed)
    ...
    neighbor = site.neighbors[site.flow_direction]
    site.add_move(site.flow_direction) unless neighbor.neutral? && neighbor.strength >= site.strength
  end
